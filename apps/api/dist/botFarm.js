...
exports.getBotFarmStatusSnapshot = getBotFarmStatusSnapshot;
...
const botFarmStatus = {};
function patchBotFarmStatus(symbol, patch) {
...
function getBotFarmStatusSnapshot() {
    const now = Date.now();
    return {
        ok: true,
        startedAt: botFarmStartedAt,
        now,
        symbols: Object.fromEntries(Object.entries(botFarmStatus).map(([symbol, s]) => [
            symbol,
            {
                ...s,
                oracle: s.oracle
                    ...
                    : undefined,
                lastLoopAgeMs: typeof s.lastLoopTs === "number" ? now - s.lastLoopTs : null,
                lastBookRefreshAgeMs: typeof s.lastBookRefreshTs === "number"
                    ? now - s.lastBookRefreshTs
                    : null,
                lastSyntheticTradeAgeMs: typeof s.lastSyntheticTradeTs === "number"
...
async function startOracleWatcher(symbol, productId) {
...
async function scrubPaperOutliers(symbol, cfg) {
    let lo = cfg.min;
    let hi = cfg.max;
...
async function initState(symbol, cfg) {
...
async function cleanupSymbolBook(symbol, cfg, mid, nowMs) {
    ...
    if (tradeCount > cfg.tradeLimit) {
        ...
        if (olds.length) {
            ...
            const orderIds = Array.from(new Set(olds.flatMap((t) => [t.buyOrderId, t.sellOrderId]).map((x) => x.toString()))).map((x) => BigInt(x));
            await prisma.order.deleteMany({
                where: {
                    id: { in: orderIds },
                    symbol,
...
async function placeOpenDepth(symbol, cfg, mid, lo, hi, tick, userIds, regime) {
    if (userIds.length < 2)
        return;
    const [maker1, maker2] = pickTwoDistinct(userIds);
    const levels = regime === "panic"
        ? cfg.openLevels + 1
        : regime === "active"
            ? cfg.openLevels
...
async function emitPaperTrade(symbol, cfg, mid, lo, hi, tick, userIds, regime) {
    if (userIds.length < 2)
        return null;
    const [buyer, seller] = pickTwoDistinct(userIds);
    const qty = qtyFor(symbol);
...
async function startSymbolWorker(symbol, tickDb) {
    ...
    while (true) {
        try {
            ...
            // Anchor random walk
            let anchorStep = randn() * range * cfg.anchorNoisePct * (0.10 * volMult);
            ...
            s.momentum = clamp(s.momentum * 0.86 +
                randn() * tick * cfg.wickTicks * 0.04 * volMult, -momCap, momCap);
            // Mid update
            const prevMid = s.mid;
            let mid = s.mid;
            ...
            if (shouldTrade) {
                const pxNum = await emitPaperTrade(symbol, cfg, mid, lo, hi, tick, userIds, s.regime);
