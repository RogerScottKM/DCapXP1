...
exports.AdmissionControlError = void 0;
exports.resetAdmissionControlCountersForTests = resetAdmissionControlCountersForTests;
exports.computePriceDeviationBps = computePriceDeviationBps;
exports.assertWithinPriceBand = assertWithinPriceBand;
exports.assertSymbolEnabled = assertSymbolEnabled;
exports.consumeSlidingWindowLimit = consumeSlidingWindowLimit;
exports.enforceAdmissionControls = enforceAdmissionControls;
const library_1 = require("@prisma/client/runtime/library");
class AdmissionControlError extends Error {
    status = 429;
...
exports.AdmissionControlError = AdmissionControlError;
const orderWindowState = new Map();
...
function resetAdmissionControlCountersForTests() {
    orderWindowState.clear();
}
function computePriceDeviationBps(referencePrice, submittedPrice) {
    const reference = new library_1.Decimal(referencePrice);
    const submitted = new library_1.Decimal(submittedPrice);
    if (reference.lte(0))
...
function assertWithinPriceBand(input) {
    if (input.maxDeviationBps <= 0)
        return;
    const deviation = computePriceDeviationBps(input.referencePrice, input.submittedPrice);
    if (deviation.gt(input.maxDeviationBps)) {
        throw new AdmissionControlError(`Price band exceeded for ${input.symbol}: ${deviation.toFixed(2)}bps > ${input.maxDeviationBps}bps`);
    }
}
function assertSymbolEnabled(input) {
...
function consumeSlidingWindowLimit(input) {
    ...
    if (existing.timestamps.length >= input.limit) {
        throw new AdmissionControlError(`Rate limit exceeded for ${input.key}: ${existing.timestamps.length}/${input.limit} in ${windowMs}ms`);
    }
...
async function enforceAdmissionControls(input) {
    ...
    const latestTrade = tradeRepo?.findFirst
