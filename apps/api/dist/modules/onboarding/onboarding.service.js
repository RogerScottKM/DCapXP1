...
class OnboardingService {
    async getMyOnboardingStatus(userId) {
        const [user, consents, latestKycCase, paymentMethodCount, latestWalletWhitelist, latestAssessmentRun, aptivioProfile, advisorAssignment, referralAttribution, rewardBalances,] = await Promise.all([
            ...
            db_1.prisma.referralAttribution.findUnique({
            ...
            db_1.prisma.userRewardBalance.findMany({
                where: { userId },
            }),
        ]);
        ...
        const kycInProgress = latestKycCase?.status === "SUBMITTED" ||
            latestKycCase?.status === "UNDER_REVIEW";
        const kycFailed = latestKycCase?.status === "REJECTED" ||
            latestKycCase?.status === "NEEDS_INFO";
        ...
        const referralApplied = Boolean(referralAttribution);
        const pointsBalanceRow = rewardBalances.find((row) => row.unitType === "POINTS");
        const pointsBalance = pointsBalanceRow
            ? Number(pointsBalanceRow.balance)
            : null;
        const otpVerified = Boolean(user?.emailVerifiedAt || user?.phoneVerifiedAt);
        const steps = [
            ...
            (0, onboarding_mapper_1.makeStep)("CONTACT_VERIFIED", "OTP verified", user?.phoneVerifiedAt || user?.emailVerifiedAt
                ? "COMPLETED"
                : "NOT_STARTED", true, (0, utc_1.toUtcIso)(user?.phoneVerifiedAt || user?.emailVerifiedAt)),
            (0, onboarding_mapper_1.makeStep)("REFERRAL_CODE_APPLIED", "Referral code applied", referralApplied ? "COMPLETED" : "NOT_STARTED", false, (0, utc_1.toUtcIso)(referralAttribution?.attributedAt), referralAttribution
                ? {
                    referralCode: referralAttribution.referralCode.code,
                    attributionStatus: referralAttribution.status,
                }
                : undefined),
            (0, onboarding_mapper_1.makeStep)("CONSENTS_ACCEPTED", "Required consents accepted", hasAllRequiredConsents
                ? "COMPLETED"
                : acceptedConsentTypes.length
                    ? "IN_PROGRESS"
                    : "NOT_STARTED", true, consents.length ? (0, utc_1.toUtcIso)(consents[0].acceptedAt) : null, {
            ...
            (0, onboarding_mapper_1.makeStep)("KYC_SUBMITTED", "KYC submitted", kycSubmitted ? "COMPLETED" : "NOT_STARTED", true, (0, utc_1.toUtcIso)(latestKycCase?.createdAt), latestKycCase
                ? { caseStatus: latestKycCase.status, kycCaseId: latestKycCase.id }
                : undefined),
            (0, onboarding_mapper_1.makeStep)("KYC_APPROVED", "KYC approved", kycApproved
                ? "COMPLETED"
                : kycFailed
                    ? "FAILED"
                    : kycInProgress
                        ? "IN_PROGRESS"
                        : "NOT_STARTED", true, (0, utc_1.toUtcIso)(latestKycCase?.updatedAt), latestKycCase
                ? { caseStatus: latestKycCase.status }
                : undefined),
            ...
            (0, onboarding_mapper_1.makeStep)("WALLET_WHITELIST_APPROVED", "Wallet approved", walletApproved
                ? "COMPLETED"
                : walletInProgress
                    ? "IN_PROGRESS"
                    : "NOT_STARTED", false, (0, utc_1.toUtcIso)(latestWalletWhitelist?.approvedAt ?? latestWalletWhitelist?.createdAt), latestWalletWhitelist
                ? { status: latestWalletWhitelist.status }
                : undefined),
            (0, onboarding_mapper_1.makeStep)("APTIVIO_ASSESSMENT_COMPLETED", "Aptivio assessment completed", assessmentCompleted
                ? "COMPLETED"
                : latestAssessmentRun
                    ? "IN_PROGRESS"
                    : "NOT_STARTED", true, (0, utc_1.toUtcIso)(latestAssessmentRun?.scoredAt || latestAssessmentRun?.submittedAt)),
            ...
            (0, onboarding_mapper_1.makeStep)("ADVISOR_LINKED", "Advisor linked", advisorLinked ? "COMPLETED" : "NOT_STARTED", false, (0, utc_1.toUtcIso)(advisorAssignment?.createdAt), advisorAssignment
        ...
        const nextRecommendedAction = currentStep === "CONTACT_VERIFIED"
            ? {
                code: "VERIFY_CONTACT",
                label: "Verify your email or phone",
                path: "/app/verify-contact",
            }
            : currentStep === "CONSENTS_ACCEPTED"
                ? {
                    code: "COMPLETE_CONSENTS",
                    label: "Accept required consents",
                    path: "/app/consents",
                }
                : currentStep === "PROFILE_COMPLETED"
                    ...
                    : currentStep === "KYC_SUBMITTED" || currentStep === "KYC_APPROVED"
                        ? {
                            code: "VISIT_KYC",
                            label: "Review your KYC status",
                            path: "/app/kyc",
                        }
                        : currentStep === "APTIVIO_ASSESSMENT_COMPLETED"
                            ? {
                                code: "START_APTIVIO",
                                label: "Complete Aptivio assessment",
                                path: "/app/aptivio",
                            }
                            : null;
        return {
            ...
            entities: {
                ...
                referral: {
                    canApplyReferralCode: !referralAttribution,
                    referredByCodeInput: user?.referredByCodeInput ?? null,
                    appliedCode: referralAttribution?.referralCode.code ?? null,
                    referrerUserId: referralAttribution?.referrerUserId ?? null,
                    attributionStatus: referralAttribution?.status ?? null,
                    pointsBalance,
                },
            },
