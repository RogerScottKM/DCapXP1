...
const parseTradeMode = (v) => {
    if (!v)
        return undefined;
    const u = v.toUpperCase();
    if (u === "PAPER")
        return client_1.TradeMode.PAPER;
    if (u === "LIVE")
        return client_1.TradeMode.LIVE;
    return undefined;
};
// near the top of apps/api/src/routes/market.ts (after imports)
const q1 = (v) => typeof v === "string" ? v : Array.isArray(v) ? v[0] : undefined;
...
router.get("/orderbook", async (req, res) => {
    try {
        ...
        const depth = (0, marketShared_1.parsePositiveInt)(req.query.depth, 20, { min: 1, max: 500 });
        const flags = featureFlags_1.featureFlags.get(symbol);
        ...
        const level = requested === undefined || requested === null || String(requested).trim() === ""
            ...
            : (0, marketShared_1.parseBookLevel)(requested, flags.orderbookDefaultLevel);
        // L3 gating
        if (level === 3 && !(0, adminKey_1.isAdmin)(req) && !flags.publicAllowL3) {
            return res.status(403).json({
                ...
                symbol,
                code: "L3_DISABLED",
                error: "L3 orderbook is disabled for public requests. Provide x-admin-key or enable publicAllowL3.",
                flags: {
...
router.get("/trades", async (req, res) => {
    try {
        ...
        const sinceRaw = q1(req.query.sinceMs) ??
            q1(req.query.fromMs) ??
...
router.get("/candles", async (req, res) => {
    try {
        ...
        const durationMs = period === "1h" ? 60 * 60 * 1000 :
            period === "4h" ? 4 * 60 * 60 * 1000 :
                period === "24h" ? 24 * 60 * 60 * 1000 :
                    period === "7d" ? 7 * 24 * 60 * 60 * 1000 :
                        period === "30d" ? 30 * 24 * 60 * 60 * 1000 :
                            24 * 60 * 60 * 1000;
        const intervalMs = durationMs <= 24 * 60 * 60 * 1000 ? 5 * 60 * 1000 :
        ...
        const latest = await prisma_1.prisma.trade.findFirst({
            where: { symbol },
            orderBy: { createdAt: "desc" },
            select: { price: true },
        });
...
router.get("/positions", async (req, res) => {
    try {
        const userId = q1(req.query.userId) ??
            req.userId ??
            req.ctx?.user?.id ??
            undefined;
        const mode = parseTradeMode(q1(req.query.mode)) ?? client_1.TradeMode.PAPER;
        ...
        const rows = await prisma_1.prisma.balance.findMany({
            where: { userId, mode },
            orderBy: { asset: "asc" },
            select: {
                asset: true,
