...
for (const prefix of ["/api/orders"]) { app.use(prefix, ordersRoutes); } for (const prefix of ["/api/admin/reconciliation"]) { app.use(prefix, reconciliationRoutes); } app.use((req, res) => {
