...
class OnboardingService {
  async getMyOnboardingStatus(userId: string): Promise<OnboardingStatusResponse> {
    ...
    const acceptedConsentTypes = Array.from(
      new Set(consents.map((c) => c.consentType))
    );
    ...
    const kycInProgress =
      latestKycCase?.status === "SUBMITTED" ||
      latestKycCase?.status === "UNDER_REVIEW";
    const kycFailed =
      latestKycCase?.status === "REJECTED" ||
    ...
    const pointsBalanceRow = rewardBalances.find(
      (row) => row.unitType === "POINTS"
    );
    ...
    const steps = [
      ...
      makeStep(
        ...
        referralApplied ? "COMPLETED" : "NOT_STARTED",
        false,
        toUtcIso(referralAttribution?.attributedAt),
        referralAttribution
          ? {
