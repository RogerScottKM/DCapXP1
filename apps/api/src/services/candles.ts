...
export type CandlePeriod = "24h" | "7d" | "30d";

type Candle = { t: number; o: number; h: number; l: number; c: number; v: number };

export const CandleService = {
  async getCandles(symbol: string, period: CandlePeriod) {
    const now = Date.now();
...
function buildCandles(
  ...
  if (!trades.length) return syntheticCandles(startMs, endMs, intervalMs, seedKey);
...
function syntheticCandles(startMs: number, endMs: number, intervalMs: number, seedKey: string): Candle[] {
