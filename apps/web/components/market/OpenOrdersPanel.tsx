  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="max-h-64 overflow-auto text-xs">
        ...
        {rows.map((o) => (
          <div key={String(o.id)} className="grid grid-cols-5 gap-2 border-t border-white/5 py-2 text-slate-200">
            <div>{String(o.id)}</div>
        ...
        {!rows.length && <div className="pt-2 text-slate-400">No open orders</div>}
      </div>
    </div>
  );
}
