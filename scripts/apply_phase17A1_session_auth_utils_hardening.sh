...
scripts["test:auth:session-utils"] = "vitest run test/session-auth.utils.test.ts"
pkg_path.write_text(json.dumps(pkg, indent=2) + "\n")

test_path = root / "apps/api/test/session-auth.utils.test.ts"
...
print(f"Patched package.json and added {test_path.relative_to(root)} for Phase 17 A1.")
PY

echo "Resolved repo root: $ROOT"
echo "Phase 17 A1 patch applied."
