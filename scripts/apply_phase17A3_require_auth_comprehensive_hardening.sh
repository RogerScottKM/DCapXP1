...
if not pkg_path.exists():
    raise SystemExit(f"package.json not found at {pkg_path}")

pkg = json.loads(pkg_path.read_text())
scripts = pkg.setdefault("scripts", {})
scripts["test:auth:require-auth"] = "vitest run test/require-auth.comprehensive.test.ts"
pkg_path.write_text(json.dumps(pkg, indent=2) + "\n")

test_path = root / "apps/api/test/require-auth.comprehensive.test.ts"
...
print("Patched package.json and wrote apps/api/test/require-auth.comprehensive.test.ts for Phase 17 A3.")
PY

echo "Resolved repo root: $ROOT"
echo "Phase 17 A3 patch applied."
