...
function writeTempSecretIdFile(value: string) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dcapx-vault-test-"));
  TEMP_DIRS.push(dir);
  const file = path.join(dir, "secret-id.txt");
...
describe("vault-client", () => {
  ...
  it("isVaultEnabled treats common truthy and falsy values correctly", () => {
    for (const value of ["1", "true", "yes", "on", " TRUE "]) {
      process.env.VAULT_ENABLED = value;
  ...
  it("getVaultBootstrapConfig returns null when vault bootstrap is disabled", () => {
    expect(getVaultBootstrapConfig()).toBeNull();
  });

  it("getVaultBootstrapConfig reads the secret id from a file when configured", () => {
    ...
    process.env.VAULT_MOUNT_PATH = "custom-approle";

    expect(getVaultBootstrapConfig()).toEqual({
      enabled: true,
  ...
  it("fetchSecretsFromVault logs in with AppRole, reads kv-v2 secrets, and revokes the token", async () => {
    process.env.VAULT_ENABLED = "1";
    process.env.VAULT_ADDR = "http://vault:8200";
    process.env.VAULT_ROLE_ID = "role-123";
    process.env.VAULT_SECRET_ID = "secret-456";

    clientMock.write.mockResolvedValue({ auth: { client_token: "vault-token-1" } });
    ...
    await expect(fetchSecretsFromVault()).resolves.toEqual({
      JWT_SECRET: "from-vault",
      TOKEN_TTL_SECONDS: "900",
    });

    expect(nodeVaultMock).toHaveBeenCalledWith({ endpoint: "http://vault:8200", apiVersion: "v1" });
    expect(clientMock.write).toHaveBeenCalledWith("auth/approle/login", {
      role_id: "role-123",
      secret_id: "secret-456",
    });
    expect(clientMock.token).toBe("vault-token-1");
  ...
  it("fetchSecretsFromVault revokes the token even when the secret read fails", async () => {
    process.env.VAULT_ENABLED = "1";
    process.env.VAULT_ADDR = "http://vault:8200";
    process.env.VAULT_ROLE_ID = "role-123";
    process.env.VAULT_SECRET_ID = "secret-456";

    clientMock.write.mockResolvedValue({ auth: { client_token: "vault-token-2" } });
    clientMock.read.mockRejectedValue(new Error("boom: read failed"));

    await expect(fetchSecretsFromVault()).rejects.toThrow("boom: read failed");
    expect(clientMock.tokenRevokeSelf).toHaveBeenCalledTimes(1);
  });

  it("fetchSecretsFromVault rejects when AppRole login does not return a client token", async () => {
    process.env.VAULT_ENABLED = "1";
    process.env.VAULT_ADDR = "http://vault:8200";
    process.env.VAULT_ROLE_ID = "role-123";
    process.env.VAULT_SECRET_ID = "secret-456";

    clientMock.write.mockResolvedValue({ auth: {} });

    await expect(fetchSecretsFromVault()).rejects.toThrow("Vault AppRole login did not return a client token");
  });
});
...
describe("bootstrapSecrets", () => {
  ...
  it("logs that bootstrap is disabled when VAULT_ENABLED is off", async () => {
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

    await bootstrapSecrets();

    expect(logSpy).toHaveBeenCalledWith("[vault] bootstrap disabled");
  });

  it("applies only non-reserved, previously unset secrets by default", async () => {
    process.env.VAULT_ENABLED = "1";
    process.env.VAULT_ADDR = "http://vault:8200";
    process.env.VAULT_ROLE_ID = "role-123";
    process.env.VAULT_SECRET_ID = "secret-456";
    process.env.EXISTING_VALUE = "keep-me";
    process.env.EMPTY_VALUE = "";

    clientMock.write.mockResolvedValue({ auth: { client_token: "vault-token-3" } });
    clientMock.read.mockResolvedValue({
      data: {
        data: {
          JWT_SECRET: "from-vault",
          ...
          EMPTY_VALUE: "fill-me",
          VAULT_ADDR: "do-not-override-reserved",
        },
      },
    });
    ...
    expect(process.env.EXISTING_VALUE).toBe("keep-me");
    expect(process.env.EMPTY_VALUE).toBe("fill-me");
  ...
  it("overrides existing env values when VAULT_OVERRIDE_ENV is truthy", async () => {
    process.env.VAULT_ENABLED = "1";
    process.env.VAULT_ADDR = "http://vault:8200";
    process.env.VAULT_ROLE_ID = "role-123";
    process.env.VAULT_SECRET_ID = "secret-456";
    process.env.VAULT_OVERRIDE_ENV = "true";
    process.env.EXISTING_VALUE = "keep-me";
    process.env.VAULT_SECRET_PATH = "secret/data/dcapx/passb";

    clientMock.write.mockResolvedValue({ auth: { client_token: "vault-token-4" } });
    clientMock.read.mockResolvedValue({
      data: {
        data: {
          EXISTING_VALUE: "replaced-from-vault",
        },
      },
    });
    ...
    expect(process.env.EXISTING_VALUE).toBe("replaced-from-vault");
    expect(logSpy).toHaveBeenCalledWith("[vault] loaded 1 secret values from secret/data/dcapx/passb");
  });
});
''').lstrip()
...
(root / "apps/api/test/vault-bootstrap.test.ts").write_text(vault_test)
(root / "apps/api/test/ibac.middleware.test.ts").write_text(ibac_test)

print("Patched package.json and wrote apps/api/test/vault-bootstrap.test.ts and apps/api/test/ibac.middleware.test.ts for Pass B.")
PY
