...
migration_sql = dedent('''-- Phase 3B: persistent idempotency keys for order placement and cancel requests.
...
helper_ts = dedent('''import { createHash } from "node:crypto";
...
if 'withIdempotency("HUMAN_ORDER_PLACE"' not in orders_text:
    target = '  auditPrivilegedRequest("ORDER_PLACE_REQUESTED", "ORDER"),\n  async (req, res) => {'
    replacement = '  auditPrivilegedRequest("ORDER_PLACE_REQUESTED", "ORDER"),\n  withIdempotency("HUMAN_ORDER_PLACE", async (req, res) => {'
    if target not in orders_text:
        raise SystemExit("Could not patch orders.ts placement handler with idempotency.")
    orders_text = orders_text.replace(target, replacement, 1)

if 'withIdempotency("HUMAN_ORDER_PLACE"' in orders_text and '  }),
);

router.post(
  "/:orderId/cancel",' not in orders_text:
    target = '  },\n);\n\nrouter.post(\n  "/:orderId/cancel",'
    replacement = '  }),\n);\n\nrouter.post(\n  "/:orderId/cancel",'
    orders_text = orders_text.replace(target, replacement, 1)

if 'withIdempotency("HUMAN_ORDER_CANCEL"' not in orders_text:
    target = '  auditPrivilegedRequest("ORDER_CANCEL_REQUESTED", "ORDER", (req) =>\n    String(req.params.orderId),\n  ),\n  async (req, res) => {'
    replacement = '  auditPrivilegedRequest("ORDER_CANCEL_REQUESTED", "ORDER", (req) =>\n    String(req.params.orderId),\n  ),\n  withIdempotency("HUMAN_ORDER_CANCEL", async (req, res) => {'
    if target not in orders_text:
        raise SystemExit("Could not patch orders.ts cancel handler with idempotency.")
    orders_text = orders_text.replace(target, replacement, 1)

if 'withIdempotency("HUMAN_ORDER_CANCEL"' in orders_text and '  }),
);

router.get("/:orderId", async (req, res) => {' not in orders_text:
