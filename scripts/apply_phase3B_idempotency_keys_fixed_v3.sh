...
place_from = '''  auditPrivilegedRequest("ORDER_PLACE_REQUESTED", "ORDER"),
  async (req, res) => {'''
place_to = '''  auditPrivilegedRequest("ORDER_PLACE_REQUESTED", "ORDER"),
  withIdempotency("HUMAN_ORDER_PLACE", async (req, res) => {'''
if place_from in orders_text:
    orders_text = orders_text.replace(place_from, place_to, 1)
...
router.post(
  "/:orderId/cancel",'''
if place_close_from in orders_text:
    orders_text = orders_text.replace(place_close_from, place_close_to, 1)

cancel_from = '''  auditPrivilegedRequest("ORDER_CANCEL_REQUESTED", "ORDER", (req) =>
    String(req.params.orderId),
  ),
  async (req, res) => {'''
cancel_to = '''  auditPrivilegedRequest("ORDER_CANCEL_REQUESTED", "ORDER", (req) =>
    String(req.params.orderId),
  ),
  withIdempotency("HUMAN_ORDER_CANCEL", async (req, res) => {'''
if cancel_from in orders_text:
    orders_text = orders_text.replace(cancel_from, cancel_to, 1)
...
router.get("/:orderId", async (req, res) => {'''
if cancel_close_from in orders_text:
    orders_text = orders_text.replace(cancel_close_from, cancel_close_to, 1)
