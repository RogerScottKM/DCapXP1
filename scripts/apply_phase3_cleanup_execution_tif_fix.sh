...
old = """    assertValidTransition(ORDER_STATUS.OPEN, ORDER_STATUS.CANCELLED);
    await db.order.update({"""
new = """    const currentDerivedStatus = deriveOrderStatus(
      takerOrder.status,
      takerOrder.qty,
      executed,
    );
    ...
    await db.order.update({"""
if old in exec_text:
    exec_text = exec_text.replace(old, new, 1)
...
print("Patched execution.ts final cancel transition validation and rewrote apps/api/test/ledger.execution.phase3-cleanup.test.ts with stronger mocks.")
PY
