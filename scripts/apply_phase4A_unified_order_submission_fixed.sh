...
(engine_port := matching_dir / "engine-port.ts").write_text(dedent("""\
...
(db_engine := matching_dir / "db-matching-engine.ts").write_text(dedent("""\
import type { PrismaClient, Prisma } from "@prisma/client";
...
(submit_service := matching_dir / "submit-limit-order.ts").write_text(dedent("""\
...
if count == 0:
    # fallback older shape with res.json
    orders_pattern2 = re.compile(
        r'const result = await prisma\.\$transaction\(async \(tx\) => \{.*?\n\s*\}\);\n\n\s*return res\.json\(\{ ok: true, \.\.\.result \}\);',
        re.DOTALL,
    )
    orders_text, count2 = orders_pattern2.subn(
        orders_replacement.replace('return res.status(201).json', 'return res.json'),
        orders_text,
        count=1,
    )
    if count2 == 0 and 'source: "HUMAN"' not in orders_text:
...
test_path = root / "apps/api/test/order-submission-unification.test.ts"
