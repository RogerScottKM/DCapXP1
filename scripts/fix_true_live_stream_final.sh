...
echo "==> Fixing backend stream route ..."
python3 - <<'PY'
...
if old not in text:
    print("Backend stream route already patched or signature not found.")
else:
...
echo "==> Writing simple Next SSE proxy ..."
...
if old not in text:
    print("Could not find EventSource URL to patch.")
else:
    text = text.replace(old, new, 1)
    p.write_text(text)
...
echo '  curl -i -N --max-time 5 "http://127.0.0.1:4010/v1/stream/RVAI-USD?mode=PAPER"'
echo '  curl -i -N --max-time 5 "https://dcapitalx.com/api/stream/BTC-USD?mode=PAPER"'
echo '  curl -i -N --max-time 5 "https://dcapitalx.com/api/stream/RVAI-USD?mode=PAPER"'
