...
contains "$IBAC_TEST" 'rejects when required agent auth headers are missing' "ibac test covers missing headers"
...
contains "$IBAC_TEST" 'rejects invalid signatures' "ibac test covers signature rejection"
contains "$IBAC_TEST" 'attaches the principal for a valid signed request' "ibac test covers valid principal attachment"
contains "$IBAC_TEST" 'bumps mandate usage counters via upsert helpers' "ibac test covers usage bump helpers"
