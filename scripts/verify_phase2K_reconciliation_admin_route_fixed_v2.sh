...
contains "$ROUTE" 'import { runReconciliation } from "../workers/reconciliation";' "reconciliation route imports runReconciliation from worker"
contains "$ROUTE" 'const failures = results.filter((r: { ok: boolean }) => !r.ok);' "reconciliation route types the failure filter"
...
contains "$WORKER" 'startReconciliationWorker' "worker exports startReconciliationWorker"
contains "$WORKER" 'stopReconciliationWorker' "worker exports stopReconciliationWorker"
