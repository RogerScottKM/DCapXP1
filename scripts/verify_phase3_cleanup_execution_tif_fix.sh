...
contains "$EXEC" 'const currentDerivedStatus = deriveOrderStatus(' "execution.ts derives the current status before final cancel transition"
contains "$EXEC" 'assertValidTransition(currentDerivedStatus, ORDER_STATUS.CANCELLED);' "execution.ts validates final cancel transition using derived status"
